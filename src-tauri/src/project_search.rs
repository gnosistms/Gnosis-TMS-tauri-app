...
fn search_projects_sync(app: &AppHandle, input: SearchProjectsInput) -> Result<SearchProjectsResponse, String> {
  let normalized_query = normalize_search_text(&input.query);
  let limit = input.limit.unwrap_or(DEFAULT_SEARCH_LIMIT).clamp(1, MAX_SEARCH_LIMIT);
  ...
  if !query_tokens.is_empty() {
    ...
    for token in &query_tokens {
      ...
      for row in rows {
        let doc_id = row.map_err(|error| format!("Could not decode a project search token hit: {error}"))?;
  ...
  if !query_trigrams.is_empty() {
    ...
    for trigram in &query_trigrams {
      ...
      for row in rows {
        let doc_id = row.map_err(|error| format!("Could not decode a project search trigram hit: {error}"))?;
  ...
  if preliminary_candidates.is_empty() {
    ...
    for row in rows {
      let doc_id = row.map_err(|error| format!("Could not decode a fallback project search hit: {error}"))?;
  ...
  for (doc_id, _) in candidate_ids {
    let Some(document) = by_id_statement
      ...
      .optional()
      .map_err(|error| format!("Could not read an indexed project search document: {error}"))? else {
      continue;
    };
    ...
    let score = compute_search_score(&candidate, &normalized_query, query_token_count, query_trigram_count);
...
fn ensure_project_index_current(
  ...
  fs::create_dir_all(&repo_root)
    .map_err(|error| format!("Could not create the local project repo folder '{}': {error}", repo_root.display()))?;
  ...
  let mut indexed_repo_heads = HashMap::<String, String>::new();
  {
    ...
    for row in rows {
      let (repo_key, head_sha) = row.map_err(|error| format!("Could not decode indexed repo state: {error}"))?;
      indexed_repo_heads.insert(repo_key, head_sha);
    }
  }

  let active_repo_keys = repos.iter().map(|repo| repo.repo_key.clone()).collect::<HashSet<_>>();
  for repo in &repos {
...
fn discover_project_repos(repo_root: &Path) -> Result<Vec<RepoRecord>, String> {
    ...
    .map_err(|error| format!("Could not read the local project repo folder '{}': {error}", repo_root.display()))?
  {
    ...
    let project_title = read_project_title(&project_json_path)?.unwrap_or_else(|| repo_name.clone());
    let head_sha = git_output(&repo_path, &["rev-parse", "--verify", "HEAD"], None).unwrap_or_default();
...
fn reindex_repo(connection: &mut Connection, repo: &RepoRecord) -> Result<(), String> {
  ...
  if chapters_root.exists() {
    for entry in fs::read_dir(&chapters_root)
      .map_err(|error| format!("Could not read chapter folders for '{}': {error}", repo.repo_path.display()))?
    {
      ...
      let chapter_title = read_optional_string(&chapter_value, "title").unwrap_or_else(|| "File".to_string());
      ...
      for row_entry in fs::read_dir(&rows_root)
        .map_err(|error| format!("Could not read row files for '{}': {error}", rows_root.display()))?
      {
        let row_entry = row_entry.map_err(|error| format!("Could not read a row file entry: {error}"))?;
        ...
        for (language_code, field_value) in fields {
          ...
          let result_id = format!("{}:{}:{}:{}", repo.repo_key, chapter_id, row_id, language_code);
  ...
  transaction
    .commit()
    .map_err(|error| format!("Could not commit the project search reindex transaction: {error}"))?;
  Ok(())
}
...
fn remove_repo_from_index_tx(connection: &Connection, repo_key: &str) -> Result<(), String> {
  ...
  connection
    .execute("DELETE FROM search_documents WHERE repo_key = ?1", [repo_key])
...
fn project_search_db_path(app: &AppHandle, installation_id: i64) -> Result<PathBuf, String> {
  ...
  fs::create_dir_all(&search_dir)
    .map_err(|error| format!("Could not create the local search folder '{}': {error}", search_dir.display()))?;
  Ok(search_dir.join("project-search.sqlite3"))
}

fn open_project_search_db(db_path: &Path) -> Result<Connection, String> {
  let connection = Connection::open(db_path)
    .map_err(|error| format!("Could not open the project search database '{}': {error}", db_path.display()))?;
  connection
    .execute_batch(
...
fn compute_search_score(
  ...
  let trigram_dice = if query_trigram_count == 0 || candidate.document.trigram_count == 0 {
    ...
    (2.0 * candidate.trigram_hits as f64) / (query_trigram_count + candidate.document.trigram_count) as f64
  };
  let ordered_tokens = query_token_count > 0
    && tokens_appear_in_order(&candidate.document.search_text, &collect_unique_tokens(normalized_query));
  let prefix_bonus = query_token_count > 0
    && query_has_prefix_match(&candidate.document.search_text, &collect_unique_tokens(normalized_query));
  let length_penalty = {
...
fn query_has_prefix_match(document: &str, query_tokens: &[String]) -> bool {
  ...
  query_tokens
    .iter()
    .any(|query_token| document_tokens.iter().any(|document_token| document_token.starts_with(query_token)))
}
...
fn build_plain_text_snippet(plain_text: &str) -> String {
  ...
  let snippet = trimmed.chars().take(140).collect::<String>().trim().to_string();
  format!("{snippet}...")
}
...
fn read_language_name_map(chapter_value: &Value) -> HashMap<String, String> {
  ...
  for language in languages {
    let Some(code) = language.get("code").and_then(Value::as_str).map(str::trim).filter(|value| !value.is_empty()) else {
      continue;
    };
    let name = language
      .get("name")
...
mod tests {
  use super::{
    build_plain_text_snippet, collect_unique_trigrams, compute_search_score, normalize_search_text,
    score_to_number, CandidateDocument, IndexedDocument,
  };

  fn candidate(document_text: &str, token_hits: usize, trigram_hits: usize, trigram_count: usize) -> CandidateDocument {
    CandidateDocument {
      token_hits,
      trigram_hits,
      document: IndexedDocument {
  ...
  fn exact_phrase_scores_higher_than_near_match() {
    ...
    let exact = candidate("I like to eat dogs", 5, 12, collect_unique_trigrams("I like to eat dogs").len());
    let near = candidate("I like to see dogs", 4, 9, collect_unique_trigrams("I like to see dogs").len());
