...
pub(crate) fn find_project_repo_path(
  ...
  {
    let entry = entry.map_err(|error| format!("Could not read a project repo entry: {error}"))?;
...
mod tests {
  ...
  use crate::local_repo_sync_state::{LocalRepoSyncStateUpdate, upsert_local_repo_sync_state};
  ...
  fn project_matcher_prefers_resource_id_over_folder_name_match() {
    let repo_root = env::temp_dir().join(format!("gnosis-project-repo-paths-{}", Uuid::now_v7()));
