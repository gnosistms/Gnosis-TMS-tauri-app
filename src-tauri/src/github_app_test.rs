...
use crate::{
  ...
  github::{github_client, types::{BeginGithubAppInstallResponse, GithubAppInstallationInfo, GithubRepository}},
...
pub(crate) fn get_github_app_test_config() -> Result<GithubAppTestConfig, String> {
  ...
  Ok(GithubAppTestConfig {
    ...
    inspect_installation_template:
      broker_path_url("/api/github-app/installations/{installation_id}")?.to_string(),
...
pub(crate) fn inspect_github_app_test_installation(
  ...
  let response = with_optional_broker_token(
    client.get(broker_path_url(&format!(
      "/api/github-app/installations/{installation_id}"
    ))?),
  )
  .header("Accept", "application/json")
  .send()
  .map_err(|error| format!("Could not inspect the GitHub App installation through the broker: {error}"))?
  .error_for_status()
