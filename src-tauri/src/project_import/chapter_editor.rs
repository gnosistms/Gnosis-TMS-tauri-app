...
pub(super) fn load_gtms_editor_row_comments_sync(
  ...
  Ok(build_editor_row_comments_response(
    input.row_id,
...
fn build_editor_row_comments_response(
  row_id: String,
  comments_revision: u64,
...
fn append_editor_comment(row_value: &mut Value, comment: &StoredEditorComment) -> Result<u64, String> {
  let row_object = row_value
    .as_object_mut()
    .ok_or_else(|| "The row file is not a JSON object.".to_string())?;
...
fn delete_editor_comment(row_value: &mut Value, comment_id: &str, author_login: &str) -> Result<u64, String> {
  let row_object = row_value
    .as_object_mut()
    .ok_or_else(|| "The row file is not a JSON object.".to_string())?;
  ensure_editor_comments_defaults(row_object)?;
...
mod tests {
  ...
  fn append_editor_comment_appends_and_increments_revision() {
    ...
    let next_revision = append_editor_comment(&mut row_value, &comment)
      .expect("comment append should succeed");

    assert_eq!(next_revision, 1);
    assert_eq!(row_value["editor_comments_revision"], json!(1));
    assert_eq!(row_value["editor_comments"].as_array().map(Vec::len), Some(1));
  }
