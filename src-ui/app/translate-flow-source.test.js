...
test("chapter language updates include broker auth and repo sync metadata", () => {
  ...
  assert.match(source, /requestEditorOperation\(\{/);
  assert.match(source, /invokeQueuedEditorWriteCommand\("update_gtms_chapter_languages",\s*\{\s*sessionToken:\s*operation\.value\.sessionToken,/s);
