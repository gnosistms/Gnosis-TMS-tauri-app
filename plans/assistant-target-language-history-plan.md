...
The returned history is newest-first. Convert it to oldest-first, collapse consecutive entries with the same target text, and always keep the current visible editor text. If the visible editor text differs from the latest committed history entry, append a synthetic `current_user` working-draft entry.
